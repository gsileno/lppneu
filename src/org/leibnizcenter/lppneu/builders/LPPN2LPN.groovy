...
class LPPN2LPN {
    ...
    void mapNet(Net net, List<Net> alreadyRecordedNets = []) {
        ...
        for (subNet in net.subNets) {
            if (!alreadyRecordedNets.contains(subNet)) {
                mapNet(subNet, alreadyRecordedNets)
    ...
    Net transitionAnchoringNet(Net net) {
        ...
        if (!expressionTripleMap) {
            throw new RuntimeException("You have to initialize the triple map before")
        }
            ...
            if (expression.isPositive()) {
                for (place in expressionPlaceMap[expression]) {
                    for (input in place.inputs) {
                        Operation operation = ((LTransition) (input.source)).reconstructOperation()
                        triple.posList << operation
                    }
                }
        ...
        for (coupling in expressionTripleMap) {
            ...
            for (int i = 0; i < triple.posList.size(); i++) {
                LTransition tOperator = new LTransition(operation: triple.posList[i])
                subNet.transitionList << tOperator
            ...
            subNet.arcList << Arc.buildArc((Place) pNexus, (Transition) tNexus, ArcType.LINK)
            for (int i = 0; i < triple.negList.size(); i++) {
    ...
    void convert(Program source) {
        ...
        transitionAnchoredNet = transitionAnchoringNet(tripleAnchoredNet)
